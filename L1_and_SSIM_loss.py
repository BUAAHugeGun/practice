...
class loss(nn.Module):
    ...
    def L1_Loss(self, input, target):
        ret = (input - target).sum()  # type: float
    ...
    def SSIM_Loss(self, input, target):
        ...
        return 1 - (SSIM_L ** self.arfa) * (SSIM_C ** self.beta) * (SSIM_S ** self.gamma)

    def forward(self, input, target):
        return self.L1_Loss(input, target) * self.k_L1 + self.SSIM_Loss(input, target) * self.k_SSIM
